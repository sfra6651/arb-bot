...
async function rugby_odds(element) {
  ...
  const odds1 = 0; // "dummy value"
  const odds2 = await oddsElements[1].evaluate((el) => el.textContent);
...
(async () => {
  ...
  await page.goto(
    "https://www.tab.co.nz/sports/category/12077/esports/csgo-cs2/matches",
    {
      waitUntil: "networkidle0",
    }
  );
  await page.waitForSelector(".event-list__item.event-list__item--esport");

  const data = await page.$$(".event-list__item.event-list__item--esport");
  ...
  fs.writeFile("data/esports/TAB.json", jsonString, (err) => {
    if (err) {
